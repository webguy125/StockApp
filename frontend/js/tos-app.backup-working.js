...
class TOSApp {
  constructor() {
    ...
    this.currentPeriod = localStorage.getItem('lastPeriod') || '5d';
    this.currentInterval = localStorage.getItem('lastInterval') || '1m';
  ...
  /**
   * Reload current chart data without page refresh (seamless update)
   */
  async reloadChart() {
    ...
    try {
      ...
      let fetchUrl = `/data/${this.currentSymbol}?interval=${this.currentInterval}`;
      if (this.currentPeriod) fetchUrl += `&period=${this.currentPeriod}`;
      ...
      // Convert dates to CST for display
      const cstDates = data.map(row => convertToCST(row.Date));

      // Update candlestick data seamlessly using Plotly.update
      const update = {
        x: [cstDates],
      ...
      await Plotly.update(plotDiv, update, {}, [candleIndex]);

      console.log('Chart data updated seamlessly');
  ...
  handleTickerUpdate(data) {
    console.log(`[TICKER] Received: ${data.symbol} @ $${data.price.toFixed(2)}, interval=${this.currentInterval}`);
    ...
    if (!symbolMatches) {
      console.log(`[TICKER] Skipping - symbol mismatch`);
      return;
    }
    ...
    console.log(`[TICKER] About to update title for interval: ${this.currentInterval}`);

    // Update candle chart for all intervals
    {

    // Throttle updates to avoid overwhelming the chart (max 4 updates per second)
    const now = Date.now();
    ...
    // console.log('Ticker update:', data.symbol, data.price);
    ...
    try {
      ...
      const candlestickTrace = plotDiv.data[candlestickIndex];
      if (!candlestickTrace.close || candlestickTrace.close.length === 0) {
      ...
      const lastIndex = candlestickTrace.close.length - 1;
      const lastCandleTimeStr = candlestickTrace.x[lastIndex];

      // Parse timestamp - handle ISO format
      const lastCandleTime = new Date(lastCandleTimeStr);
      const currentTime = new Date(); // Current time in user's timezone

      // Determine candle duration based on interval
      let candleDurationMs = 24 * 60 * 60 * 1000; // default 1 day
      if (this.currentInterval === '1m') candleDurationMs = 60 * 1000;
      else if (this.currentInterval === '5m') candleDurationMs = 5 * 60 * 1000;
      else if (this.currentInterval === '15m') candleDurationMs = 15 * 60 * 1000;
      else if (this.currentInterval === '30m') candleDurationMs = 30 * 60 * 1000;
      else if (this.currentInterval === '1h') candleDurationMs = 60 * 60 * 1000;
      else if (this.currentInterval === '1d') candleDurationMs = 24 * 60 * 60 * 1000;
      else if (this.currentInterval === '1wk') candleDurationMs = 7 * 24 * 60 * 60 * 1000;
      else if (this.currentInterval === '1mo') candleDurationMs = 30 * 24 * 60 * 60 * 1000;

      // Calculate time since last candle (both are in the same timezone after parsing)
      const timeSinceLastCandle = currentTime - lastCandleTime;

      // If last candle is more than one interval old, we need a new forming candle
      const shouldAddNewCandle = timeSinceLastCandle >= candleDurationMs;

      // console.log(`Time check: Last candle=${lastCandleTime.toISOString()}, Now=${currentTime.toISOString()}, Gap=${Math.floor(timeSinceLastCandle/1000/60)}min, Interval=${this.currentInterval}`);

      if (shouldAddNewCandle) {
        // console.log(`🆕 Adding new ${this.currentInterval} candle (last candle was ${Math.floor(timeSinceLastCandle / 1000 / 60)} minutes ago)`);

        // Calculate the start time of the current candle period in UTC
        // Round down to the nearest interval boundary
        let currentCandleStart = new Date(currentTime);
        if (this.currentInterval === '1m') {
          currentCandleStart.setUTCSeconds(0, 0);
        } else if (this.currentInterval === '5m') {
          currentCandleStart.setUTCMinutes(Math.floor(currentCandleStart.getUTCMinutes() / 5) * 5, 0, 0);
        } else if (this.currentInterval === '15m') {
          currentCandleStart.setUTCMinutes(Math.floor(currentCandleStart.getUTCMinutes() / 15) * 15, 0, 0);
        } else if (this.currentInterval === '30m') {
          currentCandleStart.setUTCMinutes(Math.floor(currentCandleStart.getUTCMinutes() / 30) * 30, 0, 0);
        } else if (this.currentInterval === '1h') {
          currentCandleStart.setUTCMinutes(0, 0, 0);
        } else {
          currentCandleStart.setUTCHours(0, 0, 0, 0);
        }

        // Add a new candle with the properly aligned timestamp in ISO 8601 format
        const formattedTimestamp = currentCandleStart.toISOString().slice(0, 19) + 'Z';

        candlestickTrace.x.push(formattedTimestamp);
        candlestickTrace.open.push(data.price);
        candlestickTrace.high.push(data.price);
        candlestickTrace.low.push(data.price);
        candlestickTrace.close.push(data.price);

        // console.log(`📅 New candle at: ${formattedTimestamp}`);
        Plotly.restyle('tos-plot', {
        ...
        const oldClose = candlestickTrace.close[lastIndex];
        const oldHigh = candlestickTrace.high[lastIndex];
        const oldLow = candlestickTrace.low[lastIndex];

        // console.log(`📊 Updating ${this.currentInterval} candle ${lastIndex}: ${oldClose} -> ${data.price}`);

        // Directly mutate the trace data
        candlestickTrace.close[lastIndex] = data.price;

        let changed = oldClose !== data.price;

        // Update high if current price is higher
        if (data.price > candlestickTrace.high[lastIndex]) {
          candlestickTrace.high[lastIndex] = data.price;
          // console.log(`🔺 New high: ${oldHigh} -> ${data.price}`);
          changed = true;
        }

        // Update low if current price is lower
        if (data.price < candlestickTrace.low[lastIndex]) {
          candlestickTrace.low[lastIndex] = data.price;
          // console.log(`🔻 New low: ${oldLow} -> ${data.price}`);
          changed = true;
        }
  ...
  /**
   * Handle individual trade updates (for visualization/debugging)
   */
  handleTradeUpdate(data) {
    ...
    // Just log it for now (could add tick chart later)
  ...
  async reloadChart(forceRecreate = false) {
    ...
    // console.log(`🔍 [DEBUG] reloadChart called - Symbol: ${this.currentSymbol}, Period: ${this.currentPeriod}, Interval: ${this.currentInterval}, ForceRecreate: ${forceRecreate}`);

    // Reset user selection flag when loading a new chart
    window.userSelectedCandle = false;

    // console.log(`📊 Loading chart for ${this.currentSymbol} - Historical data + WebSocket updates`);

    // Map TOS period format to API format
    const periodMap = {
      '1d': { period: '1d', interval: this.currentInterval },
      '5d': { period: '5d', interval: this.currentInterval },
      '1mo': { period: '1mo', interval: this.currentInterval },
      '3mo': { period: '3mo', interval: this.currentInterval },
      '6mo': { period: '6mo', interval: this.currentInterval },
      'ytd': { period: 'ytd', interval: this.currentInterval },
      '1y': { period: '1y', interval: this.currentInterval },
      '5y': { period: '5y', interval: this.currentInterval }
    };

    // Validate and get period/interval (fallback to safe defaults if invalid)
    if (!periodMap[this.currentPeriod]) {
      console.warn(`Invalid period '${this.currentPeriod}', resetting to 5d/1m`);
      this.currentPeriod = '5d';
      this.currentInterval = '1m';
      this.saveChartSettings();
    }

    const { period, interval } = periodMap[this.currentPeriod];
    ...
    try {
      ...
      const fetchUrl = `/data/${this.currentSymbol}?period=${period}&interval=${interval}`;
      ...
      if (!data || data.length === 0) {
        ...
        // console.log(`✅ [DEBUG] Received ${data.length} candles. First date: ${data[0]?.Date}, Last date: ${data[data.length-1]?.Date}`);
        state.chartData = data;
      }

      const plotDiv = document.getElementById('tos-plot');
      const chartExists = plotDiv && plotDiv.data && plotDiv.data.length > 0;

      if (chartExists && !forceRecreate) {
        // Chart exists - update with historical data
        // console.log('Updating existing chart with historical data');

        const dates = data.map(d => convertToCST(d.Date));  // Convert UTC to CST
        const opens = data.map(d => d.Open);
        const highs = data.map(d => d.High);
        const lows = data.map(d => d.Low);
        const closes = data.map(d => d.Close);

        console.log(`🔍 [DEBUG] Updating chart with ${dates.length} candles. First CST date: ${dates[0]}, Last CST date: ${dates[dates.length-1]}`);

        // Update data without changing axis range (preserve user's zoom)
        await Plotly.update('tos-plot', {
          x: [dates],
          open: [opens],
          high: [highs],
          low: [lows],
        ...
        // Chart doesn't exist or force recreate - build from scratch
        // console.log('Creating new chart with historical data');
        ...
        // console.log(`🔍 [DEBUG] Creating chart with ${dates.length} candles. First CST date: ${dates[0]}, Last CST date: ${dates[dates.length-1]}`);
        ...
        const traces = [{
          ...
          hoverinfo: 'none',  // Hide tooltip but allow hover detection
          hovertemplate: '<extra></extra>'
        }];

        // Add volume bars if setting is enabled
        if (this.chartSettings && this.chartSettings.settings.showVolume) {
          // ThinkorSwim/Ford style: uniform cyan/blue volume bars
          const volumeColor = 'rgba(0, 188, 212, 0.7)'; // Cyan blue color
          ...
          traces.push({
            ...
            name: 'Volume',
            yaxis: 'y2',
            xaxis: 'x',  // Share same x-axis as price
            marker: {
              color: volumeColor
            },
            hoverinfo: 'none',  // Hide tooltip but allow hover detection
        ...
        // Build layout with support for multiple subplots
        const layout = this.calculateChartLayout();

        // Force x-axis range to match actual data (prevent 1970 default)
        if (dates.length > 0) {
          layout.xaxis.range = [dates[0], dates[dates.length - 1]];
          layout.xaxis.autorange = false;  // Keep autorange disabled permanently
          layout.xaxis.fixedrange = false;  // But allow user to zoom/pan
        }

        // Create chart with advanced interaction controls
        await Plotly.newPlot('tos-plot', traces, layout, {
          responsive: true,
          displayModeBar: true,
          ...
          editable: true,
          // Enable axis dragging for zoom
          modeBarButtonsToRemove: [],
          dragmode: 'pan'  // Default to pan mode
        });

        // Setup advanced interaction controls (Ctrl+drag to zoom, axis drag to zoom)
        this.setupAdvancedInteractions();

        // Remove cursor icons from spike lines
        this.removeCursorIcons();

        // Debug: Check for annotations/shapes with old dates
        const plotDiv = document.getElementById('tos-plot');
        if (plotDiv && plotDiv.layout) {
          if (plotDiv.layout.annotations && plotDiv.layout.annotations.length > 0) {
            console.log('🔍 [DEBUG] Annotations:', plotDiv.layout.annotations.map(a => ({ x: a.x, y: a.y })));
          }
          if (plotDiv.layout.shapes && plotDiv.layout.shapes.length > 0) {
            console.log('🔍 [DEBUG] Shapes:', plotDiv.layout.shapes.map(s => ({ x0: s.x0, x1: s.x1 })));
          }
        }

        // Hide all Plotly hover labels with CSS
        this.injectHoverHideCSS();

        // Add TOS-style data display on hover
        this.setupTOSDataDisplay();

        // Setup event handlers for drawing tools
        setupPlotlyHandlers('tos-plot');

        // Add TOS-style chart scaling (drag to zoom)
        this.setupChartScaling('tos-plot');

        // Re-add saved indicators (if any)
        await this.restoreSavedIndicators();
      }

      // console.log('✅ Chart loaded with historical data - WebSocket will provide real-time updates');
