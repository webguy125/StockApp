/**
 * Ray Line Tool
 * Draw a line that extends infinitely in one direction from a starting point
 */
...
export class RayLine {
  ...
  onMouseDown(event, chartState) {
    if (!this.isDrawing) {
      // First click - start the ray
      this.isDrawing = true;
      this.startPoint = { x: event.clientX, y: event.clientY };
      return {
        action: 'start-ray-line',
        x: event.clientX,
        y: event.clientY
      };
    } else {
      // Second click - finish the ray
      this.isDrawing = false;

      const ray = {
        action: 'finish-ray-line',
