/**
 * 1000-Tick Chart
 * Generates OHLCV bars from every 1000 trades using Coinbase WebSocket
 */
import { CanvasRenderer } from '../chart-renderers/canvas-renderer.js';

export class TickChart1000t {
  constructor() {
    // Tick chart configuration
    this.id = '1000t';
    this.name = '1000 ticks';
    this.tickThreshold = 1000;
    this.category = 'ticks';
    this.isCustom = false;

    // Chart renderer (independent instance)
    this.renderer = new CanvasRenderer('1000t');
  ...
  async initialize(symbol, socket) {
    // console.log(`📊 [1000T] Initializing for ${symbol}`);
    ...
    try {
      ...
      console.error(`❌ [1000T] Initialization error:`, error);
      return false;
    }
  }
  ...
  async loadHistoricalBars() {
    ...
    // console.log(`📥 [1000T] Fetching: ${url}`);
    ...
    try {
      ...
      // console.log(`✅ [1000T] Loaded ${this.data.length} tick bars`);

      // Render the data
      if (this.data.length > 0) {
        const success = await this.renderer.render(this.data, this.symbol);
        if (success) {
          // console.log('✅ [1000T] Chart rendered successfully');
        }
      } else {
        // console.log('⚠️ [1000T] No historical data, starting fresh');
      ...
      console.error(`❌ [1000T] Error loading data:`, error);
  ...
  subscribeToTrades() {
    if (!this.socket) {
      // console.warn(`⚠️ [1000T] No socket connection available`);
      return;
    }
    ...
    // console.log(`🔔 [1000T] Ready to receive ${this.symbol} trades`);
  }
  ...
  async startAccumulating(symbol, socket) {
    // console.log(`🔄 [1000T] Starting background accumulation for ${symbol}`);
    ...
    try {
      ...
      if (response.ok) {
        this.data = await response.json();
        // console.log(`✅ [1000T] Loaded ${this.data.length} tick bars (background)`);
      } else {
        this.data = [];
      }
      ...
      console.error(`❌ [1000T] Background init error:`, error);
      this.data = [];
      return false;
    }
  }
   ...
   */
  async activate() {
    // console.log(`▶️ [1000T] Activating chart`);
    ...
    if (this.data.length > 0) {
      const success = await this.renderer.render(this.data, this.symbol);
      if (success) {
        // console.log(`✅ [1000T] Chart rendered with ${this.data.length} bars`);
      }
      return success;
    } else {
      // console.log(`⚠️ [1000T] No data yet, rendering empty chart`);
      await this.renderer.render([], this.symbol);
      return true;
    }
  }
  ...
  handleTradeUpdate(data) {
    ...
    // console.log(`📈 [1000T] Trade received: ${data.product_id} price=${data.price} size=${data.size}`);
   ...
   */
  async completeBar() {
    // console.log(`✅ [1000T] Completing bar with ${this.currentBar.tickCount} trades`);
    ...
    // console.log(`  📦 [1000T] Bar created: O=${bar.Open} H=${bar.High} L=${bar.Low} C=${bar.Close} V=${bar.Volume.toFixed(4)}`);
    ...
    // console.log(`🔄 [1000T] Bar complete, accumulator reset`);
  }
  ...
  async saveBarToBackend(bar) {
    ...
    try {
      ...
      if (result.success) {
        // console.log(`💾 [1000T] Bar saved to backend (total: ${result.total_bars})`);
      } else {
        console.error(`❌ [1000T] Failed to save bar:`, result.error);
      }
    } catch (error) {
      console.error(`❌ [1000T] Error saving bar:`, error);
    }
  }
   ...
   */
  deactivate() {
    // console.log(`⏸️ [1000T] Deactivating (still accumulating in background)`);
