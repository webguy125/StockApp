/**
 * 100-Tick Chart
 * Generates OHLCV bars from every 100 trades using Coinbase WebSocket
 */
import { CanvasRenderer } from '../chart-renderers/canvas-renderer.js';

export class TickChart100t {
  constructor() {
    // Tick chart configuration
    this.id = '100t';
    this.name = '100 ticks';
    this.tickThreshold = 100;
    this.category = 'ticks';
    this.isCustom = false;

    // Chart renderer (independent instance)
    this.renderer = new CanvasRenderer('100t');
  ...
  async initialize(symbol, socket) {
    console.log(`📊 [100T] Initializing for ${symbol}`);
    ...
    try {
      ...
      console.error(`❌ [100T] Initialization error:`, error);
      return false;
    }
  }
  ...
  async loadHistoricalBars() {
    ...
    try {
      ...
      // console.log(`✅ [100T] Loaded ${this.data.length} tick bars`);

      // Render the data
      if (this.data.length > 0) {
        const success = await this.renderer.render(this.data, this.symbol);
        if (success) {
          console.log('✅ [100T] Chart rendered successfully');
        }
      } else {
        console.log('⚠️ [100T] No historical data, starting fresh');
      ...
      console.error(`❌ [100T] Error loading data:`, error);
  ...
  subscribeToTrades() {
    if (!this.socket) {
      console.warn(`⚠️ [100T] No socket connection available`);
      return;
    }
    ...
    console.log(`🔔 [100T] Ready to receive ${this.symbol} trades`);
  }
  ...
  async startAccumulating(symbol, socket) {
    console.log(`🔄 [100T] Starting background accumulation for ${symbol}`);
    ...
    try {
      ...
      if (response.ok) {
        this.data = await response.json();
        console.log(`✅ [100T] Loaded ${this.data.length} tick bars (background)`);
      } else {
        this.data = [];
      }
      ...
      console.error(`❌ [100T] Background init error:`, error);
      this.data = [];
      return false;
    }
  }
   ...
   */
  async activate() {
    console.log(`▶️ [100T] Activating chart`);
    ...
    if (this.data.length > 0) {
      const success = await this.renderer.render(this.data, this.symbol);
      if (success) {
        console.log(`✅ [100T] Chart rendered with ${this.data.length} bars`);
      }
      return success;
    } else {
      console.log(`⚠️ [100T] No data yet, rendering empty chart`);
      await this.renderer.render([], this.symbol);
      return true;
    }
  }
  ...
  handleTradeUpdate(data) {
    ...
    // console.log(`📈 [100T] Trade received: ${data.product_id} price=${data.price} size=${data.size}`);
    ...
    // console.log(`  📊 [100T] Current bar: ${this.currentBar.tickCount}/${this.tickThreshold} trades`);
   ...
   */
  async completeBar() {
    // console.log(`✅ [100T] Completing bar with ${this.currentBar.tickCount} trades`);
    ...
    // console.log(`  📦 [100T] Bar created: O=${bar.Open} H=${bar.High} L=${bar.Low} C=${bar.Close} V=${bar.Volume.toFixed(4)}`);
    ...
    // console.log(`🔄 [100T] Bar complete, accumulator reset`);
  }
  ...
  async saveBarToBackend(bar) {
    ...
    try {
      ...
      if (result.success) {
        // console.log(`💾 [100T] Bar saved to backend (total: ${result.total_bars})`);
      } else {
        console.error(`❌ [100T] Failed to save bar:`, result.error);
      }
    } catch (error) {
      console.error(`❌ [100T] Error saving bar:`, error);
    }
  }
   ...
   */
  deactivate() {
    console.log(`⏸️ [100T] Deactivating`);
