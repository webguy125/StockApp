...
export class Timeframe1m {
  ...
  subscribeToLiveData() {
    ...
    console.log(`🔔 [1M] Subscribed to ${this.symbol}`);
  }
  ...
  handleTickerUpdate(data) {
    ...
    console.log(`  ✅ [1M] Processing ticker update - price=${price}, volume=${volumeBTC}, data.length=${this.data.length}`);
    ...
    if (this.data.length > 0) {
      ...
      console.log(`  ⚠️ [1M] Chart not loaded yet, ticker stored for later`);
    }
  }
  ...
  handleTradeUpdate(data) {
    // Not needed for 1-minute charts
  }
