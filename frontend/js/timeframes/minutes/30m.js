...
export class Timeframe30m {
  ...
  async initialize(symbol, socket) {
    ...
    try {
      ...
      volumeAccumulator.registerCallback('30m', this.volumeCallback);
      ...
      tradierPriceUpdater.registerCallback('30m', this.priceCallback);
      ...
      this.newCandleCallback = (interval) => {
        if (this.isActive && this.data.length > 0 && interval === '30m') {
          console.log(`🕐 [30M] New candle detected - checking data array`);
          ...
          // Get current timestamp (rounded down to 30-minute boundary)
          const now = new Date();
          const candleTime = new Date(Math.floor(now.getTime() / (30 * 60000)) * (30 * 60000));
          ...
          if (lastCandleTime.getTime() === candleTime.getTime()) {
            ...
            console.log(`🗑️ [30M] Removing duplicate flat candle at ${candleTime.toLocaleTimeString()}`);
          ...
          console.log(`✅ [30M] Added candle #${this.data.length}: ${candleTime.toLocaleTimeString()} @ $${currentPrice.toFixed(2)}`);
      ...
      volumeAccumulator.registerNewCandleCallback('30m', this.newCandleCallback);
  ...
  handleTickerUpdate(data) {
    ...
    // console.log(`  ✅ [30M] Live update - price=${price}, bid=${bid}, ask=${ask}, data.length=${this.data.length}`);
  ...
  deactivate() {
    ...
    if (this.volumeCallback) {
      volumeAccumulator.unregisterCallback('30m', this.volumeCallback);
      this.volumeCallback = null;
    }

    // Unregister Tradier price callback
    if (this.priceCallback) {
      tradierPriceUpdater.unregisterCallback('30m');
      this.priceCallback = null;
    }

    // Unregister new candle callback
    if (this.newCandleCallback) {
      volumeAccumulator.unregisterNewCandleCallback('30m', this.newCandleCallback);
