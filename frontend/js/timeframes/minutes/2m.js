/**
 * 2 Minute Timeframe
 * Independent implementation - displays 2-minute candlestick chart with live updates
 */
...
export class Timeframe2m {
  constructor() {
    // Timeframe configuration
    this.id = '2m';
    this.name = '2 minutes';
    this.interval = '2m';
    this.period = '2d';  // Show 2 days of 2-minute candles
    this.category = 'minutes';
    this.isCustom = false;

    // Chart renderer (independent instance)
    this.renderer = new CanvasRenderer('2m');
  ...
  async initialize(symbol, socket) {
    console.log(`📊 [2M] Initializing for ${symbol}`);
    ...
    try {
      ...
      if (this.data.length > 0) {
        ...
        try {
          const response = await fetch(`/current-candle-volume/${symbol}?interval=2m`);
          const currentCandleData = await response.json();

          console.log(`📊 [2M] Current candle data: O=${currentCandleData.open?.toFixed(2)} H=${currentCandleData.high?.toFixed(2)} L=${currentCandleData.low?.toFixed(2)} C=${currentCandleData.close?.toFixed(2)} V=${currentCandleData.volume?.toFixed(0)}`);
          ...
          volumeAccumulator.initializeCandleTimes('2m', currentCandleData.candle_start_time);
          volumeAccumulator.initializeVolume('2m', currentCandleData.volume);
        } catch (error) {
          console.error(`❌ [2M] Failed to fetch current candle data:`, error);
          // Fallback to 0 if fetch fails
          volumeAccumulator.initializeCandleTimes('2m', lastCandle.Date);
          volumeAccumulator.initializeVolume('2m', 0);
        }
      }
      ...
      volumeAccumulator.registerCallback('2m', this.volumeCallback);
      ...
      tradierPriceUpdater.registerCallback('2m', this.priceCallback);
      ...
      this.newCandleCallback = (interval) => {
        if (this.isActive && this.data.length > 0 && interval === '2m') {
          console.log(`🕐 [2M] New candle detected - checking data array`);
          ...
          // Get current timestamp (rounded down to 2-minute boundary)
          const now = new Date();
          const candleTime = new Date(Math.floor(now.getTime() / (2 * 60000)) * (2 * 60000));
          ...
          if (lastCandleTime.getTime() === candleTime.getTime()) {
            ...
            console.log(`🗑️ [2M] Removing duplicate flat candle at ${candleTime.toLocaleTimeString()}`);
          ...
          console.log(`✅ [2M] Added candle #${this.data.length}: ${candleTime.toLocaleTimeString()} @ $${currentPrice.toFixed(2)}`);
      ...
      volumeAccumulator.registerNewCandleCallback('2m', this.newCandleCallback);
      ...
      console.error(`❌ [2M] Initialization error:`, error);
      return false;
    }
  }
  ...
  async loadHistoricalData() {
    ...
    try {
      ...
      console.log(`✅ [2M] Loaded ${this.data.length} 2-minute candles`);

      // Render the data
      const success = await this.renderer.render(this.data, this.symbol);

      if (success) {
        console.log('✅ [2M] Chart rendered successfully');

        // Apply any ticker update that arrived during chart load
        if (this.lastTickerUpdate && this.lastTickerUpdate.symbol === this.symbol) {
          console.log(`🔄 [2M] Applying pending ticker update: ${this.lastTickerUpdate.symbol} = $${this.lastTickerUpdate.price}`);
      ...
      console.error(`❌ [2M] Error loading data:`, error);
      throw error;
    }
  }
  ...
  subscribeToLiveData() {
    if (!this.socket) {
      console.warn(`⚠️ [2M] No socket connection available`);
      return;
    }
    ...
    if (isCrypto) {
      ...
      console.log(`🔔 [2M] Subscribed to ${this.symbol} ticker`);
    } else {
      console.log(`📊 [2M] Skipping WebSocket subscription for stock symbol: ${this.symbol}`);
    }
  }
  ...
  handleTickerUpdate(data) {
    ...
    if (!isCrypto) {
      console.log(`📊 [2M] Ignoring ticker update for stock symbol: ${this.symbol}`);
      return;
    }
   ...
   */
  deactivate() {
    console.log(`⏸️ [2M] Deactivating`);

    this.isActive = false;

    // Unregister volume callback
    if (this.volumeCallback) {
      volumeAccumulator.unregisterCallback('2m', this.volumeCallback);
      this.volumeCallback = null;
    }

    // Unregister Tradier price callback
    if (this.priceCallback) {
      tradierPriceUpdater.unregisterCallback('2m');
      this.priceCallback = null;
    }

    // Unregister new candle callback
    if (this.newCandleCallback) {
      volumeAccumulator.unregisterNewCandleCallback('2m', this.newCandleCallback);
