...
export class Timeframe15m {
  ...
  async initialize(symbol, socket) {
    ...
    try {
      ...
      tradierPriceUpdater.registerCallback('15m', this.priceCallback);
      ...
      this.newCandleCallback = (interval) => {
        if (this.isActive && this.data.length > 0 && interval === '15m') {
          console.log(`🕐 [15M] New candle detected - checking data array`);
          ...
          // Get current timestamp (rounded down to 15-minute boundary)
          const now = new Date();
          const candleTime = new Date(Math.floor(now.getTime() / (15 * 60000)) * (15 * 60000));
          ...
          if (lastCandleTime.getTime() === candleTime.getTime()) {
            ...
            console.log(`🗑️ [15M] Removing duplicate flat candle at ${candleTime.toLocaleTimeString()}`);
          ...
          console.log(`✅ [15M] Added candle #${this.data.length}: ${candleTime.toLocaleTimeString()} @ $${currentPrice.toFixed(2)}`);
      ...
      volumeAccumulator.registerNewCandleCallback('15m', this.newCandleCallback);
  ...
  deactivate() {
    ...
    if (this.priceCallback) {
      tradierPriceUpdater.unregisterCallback('15m');
      this.priceCallback = null;
    }

    // Unregister new candle callback
    if (this.newCandleCallback) {
      volumeAccumulator.unregisterNewCandleCallback('15m', this.newCandleCallback);
