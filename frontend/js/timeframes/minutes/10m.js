/**
 * 10 Minute Timeframe
 * Independent implementation - displays 10-minute candlestick chart with live updates
 */
...
export class Timeframe10m {
  constructor() {
    // Timeframe configuration
    this.id = '10m';
    this.name = '10 minutes';
    this.interval = '10m';
    this.period = '5d';
    ...
    this.renderer = new CanvasRenderer('10m');
  ...
  async initialize(symbol, socket) {
    console.log(`📊 [10M] Initializing for ${symbol}`);
    ...
    try {
      ...
      if (this.data.length > 0) {
        ...
        try {
          const response = await fetch(`/current-candle-volume/${symbol}?interval=10m`);
          const currentCandleData = await response.json();

          console.log(`📊 [10M] Current candle data: O=${currentCandleData.open?.toFixed(2)} H=${currentCandleData.high?.toFixed(2)} L=${currentCandleData.low?.toFixed(2)} C=${currentCandleData.close?.toFixed(2)} V=${currentCandleData.volume?.toFixed(0)}`);
          ...
          volumeAccumulator.initializeCandleTimes('10m', currentCandleData.candle_start_time);
          volumeAccumulator.initializeVolume('10m', currentCandleData.volume);
        } catch (error) {
          console.error(`❌ [10M] Failed to fetch current candle data:`, error);
          // Fallback to 0 if fetch fails
          volumeAccumulator.initializeCandleTimes('10m', lastCandle.Date);
          volumeAccumulator.initializeVolume('10m', 0);
        }
      }
      ...
      volumeAccumulator.registerCallback('10m', this.volumeCallback);
      ...
      tradierPriceUpdater.registerCallback('10m', this.priceCallback);
      ...
      this.newCandleCallback = (interval) => {
        if (this.isActive && this.data.length > 0 && interval === '10m') {
          console.log(`🕐 [10M] New candle detected - checking data array`);

          const lastCandle = this.data[this.data.length - 1];
          const currentPrice = this.lastTickerUpdate?.price || lastCandle.Close;

          // Get current timestamp (rounded down to 10-minute boundary)
          const now = new Date();
          const candleTime = new Date(Math.floor(now.getTime() / (10 * 60000)) * (10 * 60000));
          ...
          if (lastCandleTime.getTime() === candleTime.getTime()) {
            ...
            console.log(`🗑️ [10M] Removing duplicate flat candle at ${candleTime.toLocaleTimeString()}`);
          ...
          console.log(`✅ [10M] Added candle #${this.data.length}: ${candleTime.toLocaleTimeString()} @ $${currentPrice.toFixed(2)}`);
      ...
      volumeAccumulator.registerNewCandleCallback('10m', this.newCandleCallback);
      ...
      console.error(`❌ [10M] Initialization error:`, error);
      return false;
    }
  }
  ...
  async loadHistoricalData() {
    ...
    try {
      ...
      // console.log(`✅ [10M] Loaded ${this.data.length} 10-minute candles`);

      // Render the data
      const success = await this.renderer.render(this.data, this.symbol);

      if (success) {
        // console.log('✅ [10M] Chart rendered successfully');

        // Apply any ticker update that arrived during chart load
        if (this.lastTickerUpdate && this.lastTickerUpdate.symbol === this.symbol) {
          // console.log(`🔄 [10M] Applying pending ticker update: ${this.lastTickerUpdate.symbol} = $${this.lastTickerUpdate.price}`);
      ...
      console.error(`❌ [10M] Error loading data:`, error);
      throw error;
    }
  }
  ...
  subscribeToLiveData() {
    if (!this.socket) {
      console.warn(`⚠️ [10M] No socket connection available`);
      return;
    }
    ...
    if (isCrypto) {
      ...
      console.log(`🔔 [10M] Subscribed to ${this.symbol} ticker`);
    } else {
      console.log(`📊 [10M] Skipping WebSocket subscription for stock symbol: ${this.symbol}`);
    }
  }
  ...
  handleTickerUpdate(data) {
    // console.log(`📈 [10M] Received ticker: ${data.symbol} = $${data.price}, isActive=${this.isActive}`);
    ...
    if (!isCrypto) {
      console.log(`📊 [10M] Ignoring ticker update for stock symbol: ${this.symbol}`);
      return;
    }
    ...
    if (!this.isActive || !data || !symbolMatches) {
      console.log(`  ❌ [10M] Skipping ticker - isActive=${this.isActive}, symbolMatches=${symbolMatches}`);
      return;
    }
    ...
    // console.log(`  ✅ [10M] Live update - price=${price}, bid=${bid}, ask=${ask}, data.length=${this.data.length}`);
    ...
    if (this.data.length > 0) {
      // console.log(`  🖼️ [10M] Updating renderer with live price`);
      ...
      // console.log(`  ⚠️ [10M] Chart not loaded yet, ticker stored for later`);
    }
  }
   ...
   */
  deactivate() {
    console.log(`⏸️ [10M] Deactivating`);

    this.isActive = false;

    // Unregister volume callback
    if (this.volumeCallback) {
      volumeAccumulator.unregisterCallback('10m', this.volumeCallback);
      this.volumeCallback = null;
    }

    // Unregister Tradier price callback
    if (this.priceCallback) {
      tradierPriceUpdater.unregisterCallback('10m');
      this.priceCallback = null;
    }

    // Unregister new candle callback
    if (this.newCandleCallback) {
      volumeAccumulator.unregisterNewCandleCallback('10m', this.newCandleCallback);
