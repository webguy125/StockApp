/**
 * 3 Hour Timeframe
 * Independent implementation - displays 3-hour candlestick chart with live updates
 */
...
export class Timeframe3h {
  constructor() {
    ...
    this.renderer = new CanvasRenderer('3h');
  ...
  async initialize(symbol, socket) {
    console.log(`📊 [3H] Initializing for ${symbol}`);
    ...
    try {
      ...
      if (this.data.length > 0) {
        ...
        try {
          const response = await fetch(`/current-candle-volume/${symbol}?interval=3h`);
          const currentCandleData = await response.json();

          console.log(`📊 [3H] Current candle data: O=${currentCandleData.open?.toFixed(2)} H=${currentCandleData.high?.toFixed(2)} L=${currentCandleData.low?.toFixed(2)} C=${currentCandleData.close?.toFixed(2)} V=${currentCandleData.volume?.toFixed(0)}`);
          ...
          volumeAccumulator.initializeCandleTimes('3h', currentCandleData.candle_start_time);
          volumeAccumulator.initializeVolume('3h', currentCandleData.volume);
        } catch (error) {
          console.error(`❌ [3H] Failed to fetch current candle data:`, error);
          // Fallback to 0 if fetch fails
          volumeAccumulator.initializeCandleTimes('3h', lastCandle.Date);
          volumeAccumulator.initializeVolume('3h', 0);
        }
      }
      ...
      volumeAccumulator.registerCallback('3h', this.volumeCallback);
      ...
      tradierPriceUpdater.registerCallback('3h', this.priceCallback);
      ...
      this.newCandleCallback = (interval) => {
        if (this.isActive && this.data.length > 0 && interval === '3h') {
          console.log(`🕐 [3H] New candle detected - checking data array`);

          const lastCandle = this.data[this.data.length - 1];
          const currentPrice = this.lastTickerUpdate?.price || lastCandle.Close;

          // Get current timestamp (rounded down to 3-hour boundary)
          const now = new Date();
          const candleTime = new Date(Math.floor(now.getTime() / (3 * 60 * 60000)) * (3 * 60 * 60000));
          ...
          if (lastCandleTime.getTime() === candleTime.getTime()) {
            ...
            console.log(`🗑️ [3H] Removing duplicate flat candle at ${candleTime.toLocaleTimeString()}`);
          ...
          console.log(`✅ [3H] Added candle #${this.data.length}: ${candleTime.toLocaleTimeString()} @ $${currentPrice.toFixed(2)}`);
      ...
      volumeAccumulator.registerNewCandleCallback('3h', this.newCandleCallback);
      ...
      console.error(`❌ [3H] Initialization error:`, error);
      return false;
    }
  }
  ...
  async loadHistoricalData() {
    ...
    try {
      ...
      // console.log(`✅ [3H] Loaded ${this.data.length} 3-hour candles`);

      // Render the data
      const success = await this.renderer.render(this.data, this.symbol);

      if (success) {
        // console.log('✅ [3H] Chart rendered successfully');

        // Apply any ticker update that arrived during chart load
        if (this.lastTickerUpdate && this.lastTickerUpdate.symbol === this.symbol) {
          // console.log(`🔄 [3H] Applying pending ticker update: ${this.lastTickerUpdate.symbol} = $${this.lastTickerUpdate.price}`);
      ...
      console.error(`❌ [3H] Error loading data:`, error);
      throw error;
    }
  }
  ...
  subscribeToLiveData() {
    if (!this.socket) {
      console.warn(`⚠️ [3H] No socket connection available`);
      return;
    }
    ...
    if (isCrypto) {
      ...
      console.log(`🔔 [3H] Subscribed to ${this.symbol} ticker`);
    } else {
      console.log(`📊 [3H] Skipping WebSocket subscription for stock symbol: ${this.symbol}`);
    }
  }
  ...
  handleTickerUpdate(data) {
    // console.log(`📈 [3H] Received ticker: ${data.symbol} = $${data.price}, isActive=${this.isActive}`);
    ...
    if (!isCrypto) {
      console.log(`📊 [3H] Ignoring ticker update for stock symbol: ${this.symbol}`);
      return;
    }
    ...
    if (!this.isActive || !data || !symbolMatches) {
      console.log(`  ❌ [3H] Skipping ticker - isActive=${this.isActive}, symbolMatches=${symbolMatches}`);
      return;
    }
    ...
    // console.log(`  ✅ [3H] Live update - price=${price}, bid=${bid}, ask=${ask}, data.length=${this.data.length}`);
    ...
    if (this.data.length > 0) {
      // console.log(`  🖼️ [3H] Updating renderer with live price`);
      ...
      // console.log(`  ⚠️ [3H] Chart not loaded yet, ticker stored for later`);
    }
  }
   ...
   */
  deactivate() {
    console.log(`⏸️ [3H] Deactivating`);

    this.isActive = false;

    // Unregister volume callback
    if (this.volumeCallback) {
      volumeAccumulator.unregisterCallback('3h', this.volumeCallback);
      this.volumeCallback = null;
    }

    // Unregister Tradier price callback
    if (this.priceCallback) {
      tradierPriceUpdater.unregisterCallback('3h');
      this.priceCallback = null;
    }

    // Unregister new candle callback
    if (this.newCandleCallback) {
      volumeAccumulator.unregisterNewCandleCallback('3h', this.newCandleCallback);
