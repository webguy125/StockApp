...
export class Timeframe4h {
    ...
    try {
      ...
      if (this.data.length > 0) {
        ...
        try {
          ...
          console.log(`📊 [4H] Current candle data: O=${currentCandleData.open?.toFixed(2)} H=${currentCandleData.high?.toFixed(2)} L=${currentCandleData.low?.toFixed(2)} C=${currentCandleData.close?.toFixed(2)} V=${currentCandleData.volume?.toFixed(0)}`);
          ...
          console.error(`❌ [4H] Failed to fetch current candle data:`, error);
      ...
      tradierPriceUpdater.registerCallback('4h', this.priceCallback);
      ...
      this.newCandleCallback = (interval) => {
        if (this.isActive && this.data.length > 0 && interval === '4h') {
          console.log(`🕐 [4H] New candle detected - checking data array`);
          ...
          // Get current timestamp (rounded down to 4-hour boundary)
          const now = new Date();
          const candleTime = new Date(Math.floor(now.getTime() / (4 * 60 * 60000)) * (4 * 60 * 60000));
          ...
          if (lastCandleTime.getTime() === candleTime.getTime()) {
            ...
            console.log(`🗑️ [4H] Removing duplicate flat candle at ${candleTime.toLocaleTimeString()}`);
          ...
          console.log(`✅ [4H] Added candle #${this.data.length}: ${candleTime.toLocaleTimeString()} @ $${currentPrice.toFixed(2)}`);
      ...
      volumeAccumulator.registerNewCandleCallback('4h', this.newCandleCallback);
  ...
  subscribeToLiveData() {
    ...
    if (isCrypto) {
      ...
      console.log(`📊 [4H] Skipping WebSocket subscription for stock symbol: ${this.symbol}`);
    }
  }
  ...
  handleTickerUpdate(data) {
    ...
    if (!isCrypto) {
      console.log(`📊 [4H] Ignoring ticker update for stock symbol: ${this.symbol}`);
      return;
    }
  ...
  deactivate() {
    ...
    if (this.priceCallback) {
      tradierPriceUpdater.unregisterCallback('4h');
      this.priceCallback = null;
    }

    // Unregister new candle callback
    if (this.newCandleCallback) {
      volumeAccumulator.unregisterNewCandleCallback('4h', this.newCandleCallback);
