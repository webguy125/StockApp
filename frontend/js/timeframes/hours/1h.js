...
export class Timeframe1h {
  ...
  async initialize(symbol, socket) {
    ...
    try {
      ...
      if (this.data.length > 0) {
        ...
        try {
          const response = await fetch(`/current-candle-volume/${symbol}?interval=1h`);
          const currentCandleData = await response.json();

          console.log(`📊 [1H] Current candle data: O=${currentCandleData.open?.toFixed(2)} H=${currentCandleData.high?.toFixed(2)} L=${currentCandleData.low?.toFixed(2)} C=${currentCandleData.close?.toFixed(2)} V=${currentCandleData.volume?.toFixed(0)}`);
          ...
          volumeAccumulator.initializeCandleTimes('1h', currentCandleData.candle_start_time);
          volumeAccumulator.initializeVolume('1h', currentCandleData.volume);
        } catch (error) {
          console.error(`❌ [1H] Failed to fetch current candle data:`, error);
          // Fallback to 0 if fetch fails
          volumeAccumulator.initializeCandleTimes('1h', lastCandle.Date);
          volumeAccumulator.initializeVolume('1h', 0);
        }
      }
      ...
      volumeAccumulator.registerCallback('1h', this.volumeCallback);
      ...
      tradierPriceUpdater.registerCallback('1h', this.priceCallback);
      ...
      this.newCandleCallback = (interval) => {
        if (this.isActive && this.data.length > 0 && interval === '1h') {
          console.log(`🕐 [1H] New candle detected - checking data array`);
          ...
          // Get current timestamp (rounded down to 1-hour boundary)
          const now = new Date();
          const candleTime = new Date(Math.floor(now.getTime() / (60 * 60000)) * (60 * 60000));
          ...
          if (lastCandleTime.getTime() === candleTime.getTime()) {
            ...
            console.log(`🗑️ [1H] Removing duplicate flat candle at ${candleTime.toLocaleTimeString()}`);
          ...
          console.log(`✅ [1H] Added candle #${this.data.length}: ${candleTime.toLocaleTimeString()} @ $${currentPrice.toFixed(2)}`);
      ...
      volumeAccumulator.registerNewCandleCallback('1h', this.newCandleCallback);
  ...
  subscribeToLiveData() {
    ...
    if (isCrypto) {
      ...
      console.log(`🔔 [1H] Subscribed to ${this.symbol} ticker`);
    } else {
      console.log(`📊 [1H] Skipping WebSocket subscription for stock symbol: ${this.symbol}`);
    }
  }
  ...
  handleTickerUpdate(data) {
    ...
    if (!isCrypto) {
      console.log(`📊 [1H] Ignoring ticker update for stock symbol: ${this.symbol}`);
      return;
    }
    ...
    // console.log(`  ✅ [1H] Live update - price=${price}, bid=${bid}, ask=${ask}, data.length=${this.data.length}`);
  ...
  deactivate() {
    ...
    if (this.volumeCallback) {
      volumeAccumulator.unregisterCallback('1h', this.volumeCallback);
      this.volumeCallback = null;
    }

    // Unregister Tradier price callback
    if (this.priceCallback) {
      tradierPriceUpdater.unregisterCallback('1h');
      this.priceCallback = null;
    }

    // Unregister new candle callback
    if (this.newCandleCallback) {
      volumeAccumulator.unregisterNewCandleCallback('1h', this.newCandleCallback);
