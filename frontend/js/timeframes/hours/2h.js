/**
 * 2 Hour Timeframe
 * Independent implementation - displays 2-hour candlestick chart with live updates
 */
...
export class Timeframe2h {
  constructor() {
    ...
    this.renderer = new CanvasRenderer('2h');
  ...
  async initialize(symbol, socket) {
    console.log(`📊 [2H] Initializing for ${symbol}`);
    ...
    try {
      ...
      if (this.data.length > 0) {
        ...
        try {
          const response = await fetch(`/current-candle-volume/${symbol}?interval=2h`);
          const currentCandleData = await response.json();

          console.log(`📊 [2H] Current candle data: O=${currentCandleData.open?.toFixed(2)} H=${currentCandleData.high?.toFixed(2)} L=${currentCandleData.low?.toFixed(2)} C=${currentCandleData.close?.toFixed(2)} V=${currentCandleData.volume?.toFixed(0)}`);
          ...
          volumeAccumulator.initializeCandleTimes('2h', currentCandleData.candle_start_time);
          volumeAccumulator.initializeVolume('2h', currentCandleData.volume);
        } catch (error) {
          console.error(`❌ [2H] Failed to fetch current candle data:`, error);
          // Fallback to 0 if fetch fails
          volumeAccumulator.initializeCandleTimes('2h', lastCandle.Date);
          volumeAccumulator.initializeVolume('2h', 0);
        }
      }
      ...
      this.newCandleCallback = (interval) => {
        if (this.isActive && this.data.length > 0 && interval === '2h') {
          console.log(`🕐 [2H] New candle detected - checking data array`);
          ...
          // Get current timestamp (rounded down to 2-hour boundary)
          const now = new Date();
          const candleTime = new Date(Math.floor(now.getTime() / (2 * 60 * 60000)) * (2 * 60 * 60000));
          ...
          if (lastCandleTime.getTime() === candleTime.getTime()) {
            ...
            console.log(`🗑️ [2H] Removing duplicate flat candle at ${candleTime.toLocaleTimeString()}`);
          ...
          console.log(`✅ [2H] Added candle #${this.data.length}: ${candleTime.toLocaleTimeString()} @ $${currentPrice.toFixed(2)}`);
      ...
      volumeAccumulator.registerNewCandleCallback('2h', this.newCandleCallback);
      ...
      console.error(`❌ [2H] Initialization error:`, error);
      return false;
    }
  }
  ...
  async loadHistoricalData() {
    ...
    try {
      ...
      console.error(`❌ [2H] Error loading data:`, error);
      throw error;
    }
  }
  ...
  subscribeToLiveData() {
    if (!this.socket) {
      console.warn(`⚠️ [2H] No socket connection available`);
      return;
    }
    ...
    if (isCrypto) {
      ...
      console.log(`🔔 [2H] Subscribed to ${this.symbol} ticker`);
    } else {
      console.log(`📊 [2H] Skipping WebSocket subscription for stock symbol: ${this.symbol}`);
    }
  }
  ...
  handleTickerUpdate(data) {
    ...
    if (!isCrypto) {
      console.log(`📊 [2H] Ignoring ticker update for stock symbol: ${this.symbol}`);
      return;
    }
  ...
  deactivate() {
    ...
    if (this.newCandleCallback) {
      volumeAccumulator.unregisterNewCandleCallback('2h', this.newCandleCallback);
