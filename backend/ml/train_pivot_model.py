"""
Training Script for Pivot Reliability Model

Workflow:
1. Fetch historical data for multiple assets
2. Aggregate to multiple timeframes
3. Calculate indicators and features
4. Generate pivot labels (5% reversal detection)
5. Train PyTorch model with validation split
6. Save best model
"""
...
TRAINING_CONFIG = {
    # Assets to train on (stocks only for now - easier data access)
    'crypto_assets': [],  # Disabled - Coinbase Pro deprecated
    'stock_assets': ['AAPL', 'TSLA', 'NVDA', 'SPY', 'QQQ'],

    # Timeframes to include (daily only - matches available data)
    'timeframes': ['1d'],

    # Data periods
    'crypto_days': 60,  # Crypto: 60 days (not used)
    'stock_days': 1095,  # Stocks: 3 years for more training data

    # Pivot labeling
    'reversal_threshold': 0.05,  # 5% price reversal
    'reversal_bars': 10,  # Within 10 bars
...
def label_pivots(candles, pivot_indices, is_high, reversal_threshold=0.05, reversal_bars=10):
    """
    ...
    Args:
        ...
        reversal_threshold: Minimum price reversal (e.g., 0.05 = 5%)
...
def fetch_and_process_asset(asset, is_crypto, timeframes, config):
    """
    Fetch data for one asset and process all timeframes
    ...
    try:
        ...
        days = config['crypto_days'] if is_crypto else config['stock_days']
        base_timeframe = '1m' if is_crypto else '1d'

        df = fetch_historical_data(asset, days=days, timeframe=base_timeframe)
        ...
        for timeframe in timeframes:
            ...
            # For daily stocks, skip aggregation (data is already daily)
            if not is_crypto and timeframe == '1d':
                df_agg = df.copy()
            else:
                # Aggregate to timeframe
                df_agg = aggregate_ohlcv(df.copy(), timeframe)
        ...
        logger.error(f"❌ Asset processing failed for {asset}: {e}")
        return {}


def prepare_training_data(config):
    """
    Fetch and prepare training data for all assets and timeframes
    ...
    for asset in config['crypto_assets']:
        results = fetch_and_process_asset(
            asset, is_crypto=True,
    ...
    for asset in config['stock_assets']:
        results = fetch_and_process_asset(
            asset, is_crypto=False,
            timeframes=config['timeframes'],
            config=config
        )
...
def main():
    ...
    """
    logger.info("\n" + "="*60)
    logger.info("PIVOT RELIABILITY MODEL TRAINING")
    logger.info("="*60 + "\n")
    ...
    # Save model
    save_model(model, epoch=TRAINING_CONFIG['epochs'], loss=None)
