"""
Gradient Boosting Model for Advanced ML Trading System
sklearn's classic gradient boosting implementation
"""
...
class GradientBoostModel:
    """
    Gradient Boosting Classifier for trading predictions

    Key Features:
    - Classic gradient boosting algorithm
    - Strong performance with proper tuning
    - Different regularization than XGBoost/LightGBM
    - Good baseline for ensemble diversity
    """

    def __init__(self, model_path: str = "backend/data/ml_models/gradientboost"):
        """
        ...
        # Hyperparameters (baseline configuration - less restrictive)
        self.hyperparameters = {
            'n_estimators': 200,
            'learning_rate': 0.1,           # Increased from 0.05
            'max_depth': 8,                 # Increased from 5 (match XGBoost)
            'min_samples_split': 5,
            'min_samples_leaf': 2,
            'subsample': 0.8,
            'max_features': 'sqrt',
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        """
        print(f"\n{'=' * 60}")
        print(f"[TRAIN] Gradient Boosting Model")
        ...
        if validate:
            ...
            cv_scores = cross_val_score(
                GradientBoostingClassifier(**self.hyperparameters),
                X_train,
                y_train,
                cv=5,
                n_jobs=-1
    ...
    def save(self) -> bool:
        ...
        try:
            # Save model
            model_file = os.path.join(self.model_path, 'gradientboost.joblib')
