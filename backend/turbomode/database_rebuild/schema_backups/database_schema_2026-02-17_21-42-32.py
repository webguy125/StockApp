...
class TurboModeDB:
    ...
    def _run_migrations(self):
        ...
        # Migration: Add prob_hold column to active_signals (2026-02-04)
        if 'prob_hold' not in columns:
            logger.info("Running migration: Adding prob_hold column to active_signals")
            try:
                cursor.execute("ALTER TABLE active_signals ADD COLUMN prob_hold REAL")
                conn.commit()
                logger.info("[OK] Migration completed: prob_hold column added")
            except Exception as e:
                logger.error(f"Migration failed: {e}")
                conn.rollback()
        ...
        if not existing:
            ...
            cursor.execute("""
                ...
                (symbol, signal_type, confidence, entry_date, entry_price, entry_min, entry_max,
                 ...
                 market_cap, sector, prob_buy, prob_sell, prob_hold,
