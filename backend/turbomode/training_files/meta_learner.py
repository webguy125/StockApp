...
class MetaLearner:
    ...
    def __init__(self, model_path: str = None, use_gpu: bool = True):
        ...
        self.model_path = model_path or r'C:\StockApp\backend\turbomode\models\trained\meta_learner'
    ...
    def _prepare_enhanced_meta_features(self, base_predictions: Dict) -> np.ndarray:
        """
        Convert base model predictions + override features to 55-dimensional feature vector.

        Args:
            base_predictions: Dictionary with:
                - Base predictions: {'xgboost': [p_down, p_neutral, p_up], ...}
                - Override features: {'xgboost_asymmetry': float, ...}

        Returns:
            Flat 55-dimensional feature vector (24 base + 31 override)
        """
        ...
        for model_name in model_names:
            probs = base_predictions[model_name]
            features.extend([float(probs[0]), float(probs[1]), float(probs[2])])  # down, neutral, up

        # 24 per-model override features
        for model_name in model_names:
            features.append(float(base_predictions[f'{model_name}_asymmetry']))
            features.append(float(base_predictions[f'{model_name}_max_directional']))
            features.append(float(base_predictions[f'{model_name}_neutral_dominance']))

        # 7 aggregate features
        features.append(float(base_predictions['avg_asymmetry']))
        features.append(float(base_predictions['max_asymmetry']))
        features.append(float(base_predictions['avg_max_directional']))
        features.append(float(base_predictions['avg_neutral_dominance']))
        features.append(float(base_predictions['models_favor_up']))
        features.append(float(base_predictions['models_favor_down']))
        features.append(float(base_predictions['directional_consensus']))

        features = np.array(features, dtype=np.float32)

        # Verify dimension
        if len(features) != 55:
            raise ValueError(f"Expected 55 features, got {len(features)}")
    ...
    def train(self, base_predictions_train: List[Dict[str, np.ndarray]],
        ...
        """
        Train meta-learner on base model predictions (24 or 55 features).

        Supports two formats:
        1. Basic 24-feature format: 8 models × 3 probabilities
        2. Enhanced 55-feature format: 24 base + 31 override-aware features

        Args:
            base_predictions_train: List of prediction dicts (one per training sample)
                                   Each dict: {'xgboost': [p_down, p_neutral, p_up], ...}
                                   Enhanced format may include override features
        ...
        """
        # Check if predictions have override-aware features (55-feature format)
        has_override = any(isinstance(v, (int, float, np.integer, np.floating))
                          for k, v in base_predictions_train[0].items()
                          if '_asymmetry' in k or '_max_directional' in k)

        if has_override:
            # Enhanced 55-feature format
            print(f"Preparing 55-feature meta-features (with override-aware) from {len(base_predictions_train)} training samples...")
            X_meta_list = []
            for pred_dict in base_predictions_train:
                meta_feat = self._prepare_enhanced_meta_features(pred_dict)
                X_meta_list.append(meta_feat)
        else:
            # Basic 24-feature format
            print(f"Preparing 24-feature meta-features from {len(base_predictions_train)} training samples...")
            X_meta_list = []
            for pred_dict in base_predictions_train:
        ...
        expected_dim = 55 if has_override else self.META_FEATURE_DIM

        print(f"Meta-feature matrix shape: {X_meta.shape}")
        print(f"Expected: ({len(base_predictions_train)}, {expected_dim})")

        # Verify dimensions
        if X_meta.shape[1] != expected_dim:
            raise ValueError(f"Meta-feature dimension mismatch: {X_meta.shape[1]} != {expected_dim}")
        ...
        # Build feature names based on format
        if has_override:
            # 55 features: 24 base + 31 override
            model_names = ['xgboost', 'xgboost_et', 'lightgbm', 'catboost',
                          'xgboost_hist', 'xgboost_dart', 'xgboost_gblinear', 'xgboost_approx']
            feature_names = []
            # 24 base probability features
            ...
            # 24 features: 8 models × 3 probabilities
            feature_names = []
            for model_name in self.BASE_MODEL_ORDER:
                feature_names.extend([
                    f'{model_name}_prob_down',
        ...
        if base_predictions_val is not None and y_val is not None:
            if has_override:
                print(f"Preparing 55-feature validation meta-features from {len(base_predictions_val)} validation samples...")
                X_val_meta_list = []
                for pred_dict in base_predictions_val:
                    meta_feat = self._prepare_enhanced_meta_features(pred_dict)
                    X_val_meta_list.append(meta_feat)
            else:
                print(f"Preparing 24-feature validation meta-features from {len(base_predictions_val)} validation samples...")
                X_val_meta_list = []
                for pred_dict in base_predictions_val:
                    meta_feat = self.prepare_meta_features(pred_dict)
                    X_val_meta_list.append(meta_feat)
    ...
    def predict_proba(self, base_predictions: Dict[str, np.ndarray]) -> np.ndarray:
        """
        CRITICAL FIX #5: Generate probability distribution from base model predictions.
        This enables dynamic confidence calculation instead of hardcoded values.

        Args:
            base_predictions: Dict {model_name: probs_array}
                             where probs_array is shape (3,) for [down, neutral, up]

        Returns:
            Array of shape (3,) with probabilities for [down, neutral, up]
        """
        ...
        if has_override_features:
            ...
            for model_name in model_names:
                ...
                features.extend([probs_array[0], probs_array[1], probs_array[2]])
