...
def compute_labels_for_trades(trades, horizon_days, thresholds):
    ...
    for trade in trades:
        ...
        if symbol not in price_data:
            logger.warning(f"No OHLCV data for symbol {symbol}")
        ...
        if not np.any(mask):
            logger.warning(f"No OHLCV window for trade {trade_id} ({symbol})")
            labels[trade_id] = {"y_tp": 0.0, "y_dd": 0.0, "outcome": 1}
            continue
        ...
        # Remove NaNs
        window_highs = window_highs[~np.isnan(window_highs)]
        window_lows = window_lows[~np.isnan(window_lows)]

        if len(window_highs) == 0 or len(window_lows) == 0:
            logger.warning(f"NaN-only window for trade {trade_id} ({symbol})")
            labels[trade_id] = {"y_tp": 0.0, "y_dd": 0.0, "outcome": 1}
            continue
