...
def train_meta_learner(train_predictions: List[Dict], y_train: np.ndarray,
    ...
    """
    Train meta-learner V2 with override-aware features (55 features).
    ...
    """
    print("\n" + "="*80)
    print("TRAINING META-LEARNER V2 (55 FEATURES WITH OVERRIDE-AWARE)")
    print("="*80)

    # Initialize meta-learner v2
    meta_path = os.path.join(MODEL_BASE_PATH, 'meta_learner_v2')
    meta_learner = MetaLearner(model_path=meta_path, use_gpu=True)

    print(f"Training on {len(train_predictions):,} base prediction sets")
    print(f"Adding override-aware features to match production architecture...")

    # Convert predictions to enhanced format with override-aware features
    train_predictions_enhanced = add_override_features_to_predictions(train_predictions)
    val_predictions_enhanced = add_override_features_to_predictions(val_predictions)

    print(f"  Base features: 24 (8 models × 3 class probs)")
    print(f"  Override features: 31 (24 per-model + 7 aggregate)")
    print(f"  Total features: 55")

    metrics = meta_learner.train(
        train_predictions_enhanced, y_train,
        val_predictions_enhanced, y_val
    )

    # Display results
    print(f"\n[OK] Meta-learner V2 training complete:")
    print(f"   Train Accuracy: {metrics['train_accuracy']:.4f}")
    if 'val_accuracy' in metrics:
        print(f"   Val Accuracy: {metrics['val_accuracy']:.4f}")

    # Display model importance (only for 24-feature version, skip for 55-feature)
    # Feature importance is less meaningful with 55 features
...
def add_override_features_to_predictions(predictions: List[Dict]) -> List[Dict]:
    ...
    Takes 24 base features (8 models × 3 probs) and adds:
