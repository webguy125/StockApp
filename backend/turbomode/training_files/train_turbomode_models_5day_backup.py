...
def train_sector_base_models(sector: str, X_train: np.ndarray, y_train: np.ndarray,
    ...
    for i, (model_name, model_class, model_dir) in enumerate(BASE_MODELS, 1):
        try:
            # Create ABSOLUTE model output directory
            model_output_dir = os.path.join(MODEL_BASE_PATH, sector, model_dir)
...
def train_sector_meta_learner(sector: str, base_predictions: Dict[str, np.ndarray],
    ...
    # Initialize meta-learner v2 with ABSOLUTE path
    meta_output_dir = r'C:\StockApp\backend\turbomode\models\trained' + '\\' + sector + r'\meta_learner_v2'
...
def train_single_sector_worker(sector: str, X_train: np.ndarray, y_train: np.ndarray,
    ...
    # Train meta-learner
    meta_accuracy = train_sector_meta_learner(sector, base_predictions, y_val)
...
def train_all_sectors_parallel(max_workers: int = 1):
    """
