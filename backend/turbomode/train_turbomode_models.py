"""
Train TurboMode Models - Separate from Slipstream
Uses existing training data from advanced_ml_system.db
...
print("\n" + "=" * 70)
print("TRAIN TURBOMODE MODELS - INDEPENDENT FROM SLIPSTREAM")
print("=" * 70)
print(f"Start Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print(f"Shared Database: backend/data/advanced_ml_system.db")
...
# Initialize database connection (shared resource, absolute path)
# Use the FULL production dataset with 34,086 samples (78 symbols)
db_path = os.path.join(PROJECT_ROOT, "backend", "data", "advanced_ml_system.db")
print(f"\n[DATABASE] Calculated path: {db_path}")
...
print(f"[INFO] Expected: ~34,086 samples (78 symbols)")
