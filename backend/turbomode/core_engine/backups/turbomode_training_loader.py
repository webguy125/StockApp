...
def compute_labels_for_trades(trades, horizon_days, thresholds):
    """
    Compute labels dynamically from canonical candles table.
    VECTORIZED VERSION - processes trades in batches by symbol.
    ...
    # Load OHLCV data from canonical candles table
    ohlcv_data = load_ohlcv_for_trades(CANONICAL_DB_PATH, trades, horizon_days)

    # Convert DataFrames to numpy arrays (pre-parse timestamps once per symbol)
    price_data = {}
    ...
    # Group trades by symbol for batch processing
    trades_by_symbol = {}
    for trade in trades:
        symbol = trade["symbol"]
    ...
    for symbol, symbol_trades in trades_by_symbol.items():
        if symbol not in price_data:
            no_data_count += len(symbol_trades)
            for trade in symbol_trades:
                labels[trade["id"]] = {"y_tp": 0.0, "y_dd": 0.0, "outcome": 1}
            continue
        ...
        for trade in symbol_trades:
            ...
            # Extract window (no boolean mask - direct slicing)
            window_highs = highs[start_idx:end_idx]
            window_lows = lows[start_idx:end_idx]

            # Remove NaNs using numpy boolean indexing
    ...
    # Log summary instead of per-trade warnings
    if no_data_count > 0:
        logger.warning(f"No OHLCV data: {no_data_count} trades (set to HOLD)")
    if no_window_count > 0:
        logger.warning(f"No OHLCV window: {no_window_count} trades (recent entries, set to HOLD)")
    if nan_window_count > 0:
        logger.warning(f"NaN-only windows: {nan_window_count} trades (set to HOLD)")
...
class TurboModeTrainingDataLoader:
    ...
    def load_training_data(self, include_hold: bool = True, symbols_filter: Optional[List[str]] = None,
        ...
        if horizon_days is not None:
            ...
            # Compute labels dynamically from canonical candles table
            labels_by_id = compute_labels_for_trades(trades, horizon_days, thresholds)
