...
from backend.turbomode.core_engine.symbol_normalizer import validate_and_normalize, auto_correct
...
class MarketDataIngestion:
    ...
    def __init__(self, db_path: str = None):
        ...
        """
        if db_path is None:
            # Master Market Data DB is at C:\StockApp\master_market_data\market_data.db
            master_data_dir = project_root / "master_market_data"
            db_path = str(master_data_dir / "market_data.db")
    ...
    def _get_latest_candle_date(self, symbol: str, timeframe: str = '1d') -> Optional[str]:
        """
        Get the latest candle date for a symbol from the database
        ...
        Returns:
            Latest candle date as string (YYYY-MM-DD) or None if no data exists
        """
        try:
            conn = self._get_connection()
            cursor = conn.cursor()
            ...
            logger.error(f"[ERROR] Failed to get latest candle date for {symbol}: {e}")
            return None
    ...
    def ingest_candles(
        ...
        """
        Ingest OHLCV candles from yfinance (with incremental updates)

        Args:
            symbol: Stock ticker (will be normalized to canonical format)
            timeframe: Timeframe (1m, 5m, 15m, 30m, 1h, 1d, 1wk, 1mo)
            period: Period to fetch (1d, 5d, 1mo, 3mo, 6mo, 1y, 2y, 5y, 10y, ytd, max)
            start_date: Start date (YYYY-MM-DD) - overrides incremental logic
            end_date: End date (YYYY-MM-DD)
            force_full_refresh: If True, ignores existing data and fetches full period
        ...
        try:
            ...
            # Incremental update logic (only if no explicit dates provided and not forcing full refresh)
            if not start_date and not end_date and not force_full_refresh:
                latest_date = self._get_latest_candle_date(symbol, timeframe)

                if latest_date:
                    # We have existing data - only fetch new candles
                    start_date = latest_date  # yfinance will fetch from this date forward
                    logger.info(f"[INCREMENTAL] {symbol}: Fetching candles since {latest_date}")
                else:
                    # No existing data - fetch full period
                    logger.info(f"[FULL FETCH] {symbol}: No existing data, fetching {period}")

            # Fetch data
            if start_date and end_date:
                hist = ticker.history(start=start_date, end=end_date, interval=timeframe)
            elif start_date:
                hist = ticker.history(start=start_date, interval=timeframe)
            else:
...
if __name__ == '__main__':
    ...
    from backend.turbomode.training_symbols import get_training_symbols, CRYPTO_SYMBOLS
    ...
    print(f"  Scanning stocks:  {len(scanning_symbols)}")
    print(f"  Unique stocks:    {len(all_stock_symbols)}")
