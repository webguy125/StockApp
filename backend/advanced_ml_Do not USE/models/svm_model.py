"""
Support Vector Machine Model for Advanced ML Trading System
Kernel-based classification - finds optimal decision boundaries in high-dimensional space
"""
...
class SVMModel:
    """
    Support Vector Machine for trading predictions

    Uses RBF kernel to find non-linear decision boundaries
    Completely different approach from trees, linear, and neural networks
    """

    def __init__(self, model_path: str = "backend/data/ml_models/svm"):
        """
        ...
        self.model = None
        self.scaler = None  # SVMs require feature scaling
        ...
        self.hyperparameters = {
            ...
            'random_state': 42,
            'max_iter': 10000  # Increased from 1000 (needs more iterations for convergence)
        }
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        """
        print(f"\n{'=' * 60}")
        print(f"[TRAIN] Support Vector Machine Model")
        print(f"  Samples: {len(X_train)}")
        print(f"  Features: {X_train.shape[1]}")
        print(f"  Classes: {len(np.unique(y_train))}")
        print(f"  Kernel: {self.hyperparameters['kernel']}")
        print(f"  Training...")
        ...
        # Cross-validation (can be slow for SVM, so use fewer folds)
        print("  Cross-validating...")
        cv_scores = cross_val_score(
            SVC(**self.hyperparameters),
            X_scaled,
            y_train,
    ...
    def save(self) -> bool:
        ...
        try:
            # Save model
            model_file = os.path.join(self.model_path, 'svm.joblib')
...
if __name__ == '__main__':
    ...
    np.random.seed(42)
    n_samples = 500  # Smaller for SVM (it's slower)
    n_features = 179
