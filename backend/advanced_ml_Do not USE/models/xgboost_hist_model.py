...
"""
XGBoost GPU Hist Model for Trading Signals
Uses histogram-based tree construction for faster training
Optimized variant: tree_method='hist' with GPU acceleration
"""
...
class XGBoostHistModel:
    """XGBoost Hist GPU classifier - histogram-based tree construction"""

    def __init__(self, model_path: str = "backend/data/ml_models/xgboost_hist"):
        ...
        # Histogram-optimized hyperparameters
        self.hyperparameters = {
            'device': 'cuda',
            'tree_method': 'hist',         # Histogram algorithm
            'predictor': 'gpu_predictor',  # GPU accelerated inference
            'max_bin': 256,                # Histogram bins (more = slower but precise)
            'n_estimators': 300,
            'max_depth': 7,                # Slightly deeper for hist
            'learning_rate': 0.04,
            'subsample': 0.75,
    ...
    def train(self, X: np.ndarray, y: np.ndarray, X_val: np.ndarray = None, y_val: np.ndarray = None, validate: bool = False, sample_weight: np.ndarray = None) -> Dict[str, Any]:
        ...
        print("  Training on GPU with histogram method...")
        self.model.fit(
            X_train, y_train,
    ...
    def predict(self, features_dict: Dict[str, Any]) -> Dict[str, Any]:
        if not self.is_trained:
            return {'prediction': 'buy', 'buy_prob': 0.50, 'sell_prob': 0.50, 'confidence': 0.0, 'model': 'xgboost_hist_untrained'}
        ...
        return format_prediction(probabilities, prediction_class, 'xgboost_hist_gpu')
    ...
    def predict_batch(self, features_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i in range(len(predictions)):
            ...
            # Use unified prediction layer
            result = format_prediction(probs, pred_class, 'xgboost_hist_gpu')
