...
"""
XGBoost GPU DART Model for Trading Signals
Uses Dropouts meet Multiple Additive Regression Trees (DART)
Prevents overfitting by randomly dropping trees during boosting
"""
...
class XGBoostDartModel:
    """XGBoost DART GPU classifier - dropout-regularized boosting"""

    def __init__(self, model_path: str = "backend/data/ml_models/xgboost_dart"):
        ...
        # DART-optimized hyperparameters
        self.hyperparameters = {
            'device': 'cuda',
            'tree_method': 'hist',
            'predictor': 'gpu_predictor',  # GPU accelerated inference
            'booster': 'dart',             # DART booster (dropout trees)
            'sample_type': 'uniform',      # Uniform sampling for dropout
            'normalize_type': 'tree',      # Normalize by number of trees
            'rate_drop': 0.1,              # Dropout rate (10% of trees)
    ...
    def train(self, X: np.ndarray, y: np.ndarray, X_val: np.ndarray = None, y_val: np.ndarray = None, validate: bool = False, sample_weight: np.ndarray = None) -> Dict[str, Any]:
        ...
        print(f"  Booster: DART (dropout regularization)")
        ...
        print("  Training on GPU with DART booster...")
        self.model.fit(
            X_train, y_train,
    ...
    def predict(self, features_dict: Dict[str, Any]) -> Dict[str, Any]:
        if not self.is_trained:
            return {'prediction': 'buy', 'buy_prob': 0.50, 'sell_prob': 0.50, 'confidence': 0.0, 'model': 'xgboost_dart_untrained'}
        ...
        return format_prediction(probabilities, prediction_class, 'xgboost_dart_gpu')
    ...
    def predict_batch(self, features_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i in range(len(predictions)):
            ...
            # Use unified prediction layer
            result = format_prediction(probs, pred_class, 'xgboost_dart_gpu')
