...
# Import fundamental cache
from advanced_ml.features.fundamental_cache import get_fundamental_cache
...
class GPUFeatureEngineer:
    ...
    def __init__(self, use_gpu: bool = True, use_feature_selection: bool = False):
        ...
        Args:
            ...
            use_feature_selection: Whether to use only top 100 selected features (default: False to keep all 191 features including fundamentals)
        """
    ...
    def extract_features(self, df: pd.DataFrame, symbol: str = None) -> Dict[str, Any]:
        """
        Calculate all technical + fundamental features for a dataframe

        Args:
            df: DataFrame with columns: Open, High, Low, Close, Volume
            symbol: Stock symbol (required for fundamentals)

        Returns:
            Dictionary of 176 technical + 12 fundamental + 3 metadata = 191 features
        """
        if len(df) < 50:
            return {'error': 'Need at least 50 bars'}

        # Get technical features (176 features)
        features = self.calculate_features(df)

        # Add fundamental features (12 features) if symbol provided
        if symbol:
    ...
    def _get_fundamental_features(self, symbol: str) -> Dict[str, float]:
        """
        Get fundamental features from cache (fast) or yfinance (slow, cached for 24h)

        Args:
            symbol: Stock ticker

        Returns:
            Dictionary of 12 fundamental features
        """
        try:
            ...
            print(f"[WARNING] Failed to get fundamentals for {symbol}: {e}")
